...
%    X n by n image
%  Description
%    Multiply coefficients at each scale by the appropriate and  
%    Meyer window and adds contributions from all the different
%    scale. 
% See Also
%   Separatescales, Adj_Curvelet02Xform
       ...
       n = size(S(number_scales).coeff,1);
       n2 = n/2;
       J = log2(n);
       deg = 3;
       F = zeros(n);
...
%      Loop to Unfold Partition for  j = L, ..., J - 1;
	  ...
	  lx = (n2 - l + 1):(n2 + l); % ly = lx;
	  F(lx,lx) = F(lx,lx) + S(j-L+2).coeff.*w2;
       end
