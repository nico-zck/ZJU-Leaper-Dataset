...
from baselines.models.seg_transfer.misc.dual_dataset_trainer import DualDatasetTrainer
from baselines.models.seg_transfer.unet_unsup.unet_unsup import UNetTransferUnsup
from baselines.models.seg_transfer.misc.pair_mask_dsb import PairAndMaskDSB
...
if __name__ == "__main__":
    ...
    for p_id in cfg.DATASET.fabrics:
        ...
        for t in range(3):
            model = UNetTransferUnsup(cfg=cfg)
            trainer = DualDatasetTrainer(cfg=cfg, core_model=model)
            ds_builder = PairAndMaskDSB(cfg=cfg)
